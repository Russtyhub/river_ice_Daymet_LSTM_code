...
# saving the test_df dates for use later minus the first lookback window
test_dates = np.array(test_df_avg.index)[WINDOW_SIZE:]
...
for j in range(n_features):
	...
	for sample_idx in range(test_df_avg.shape[0]):
		...
		for _ in range(M):
			...
			z_index = np.random.choice(np.arange(train_X.shape[0])[WINDOW_SIZE+1:], 1)[0]
			z = train_X[z_index]
